...
def insert():
    ...
    if Name == '' or Gender == '' or Telephone == '' or Email == '':
        messagebox.showwarning('data', 'Please fill the all the fields ')
    else:
        add(data)
        messagebox.showinfo('data', 'your data is inserted')
...
def to_update():
    try:
        ...
        tree_dictonary = tree.item(tree_data)
        tree_list = tree_dictonary['values']
        ...
        def confirm():
            ...
            update(data)
            messagebox.showinfo('Success', 'data updated succesfuully')
...
def to_remove():
    try:
        ...
        messagebox.showinfo('Success', 'Succesfully data is deleted')
...
c_gender = ttk.Combobox(frame_down, width=17)
c_gender['values'] = [' ', 'Not Specified', 'Male', 'Female']
